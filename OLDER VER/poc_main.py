...
MAX_WORKERS_SCAN = 8        # 12 - full power for quick scan
MAX_WORKERS_AI = 8           # 🔥 KEY_value : number of parrallel models workings (8 max on ollama)
MAX_TEXT_LENGTH = 1000       # Lenght of text processed threw semantic analysis
BATCH_SIZE = 200             # DB batch lenght
REQUEST_TIMEOUT = 60         # Less than 60s = network error, especially for the first loaders
...
def ask_llama_worker(data_pack):
    ...
    # NOTE : L'indentation ici est cruciale. Tout ce bloc doit être DANS la fonction.
    
    # Les doubles accolades {{ }} servent à échapper les accolades dans une f-string
    # pour qu'elles apparaissent comme du JSON littéral dans le prompt final.
    promptai = f"""Détermine la nature administrative et légale du document en 1 phrase de 20 mots maximum (Contrat de prestation, Facture, Fiche de paie, Devis, Dossier)... de ce document : 
{text_snippet[:1000]}
JSON: {{"Nature": "votre résumé"}}"""
    ...
    try:
        ...
        # Ollama renvoie parfois le JSON dans une clé 'response' sous forme de string,
        # ou directement l'objet si le format json est bien géré.
        # Ici on parse le contenu de la réponse.
        content = json.loads(result_json['response'])
        return file_id, content
        
    except Exception as e:
        # En cas d'erreur, on renvoie un objet vide ou l'erreur pour ne pas casser le thread
